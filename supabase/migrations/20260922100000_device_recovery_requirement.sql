...
begin
  ...
  select id into active_recovery_key_id from public.vault_recovery_keys
  where owner_id = auth.uid() and status = 'active';
  recovery_setup_required := active_recovery_key_id is null;
  ...
  return jsonb_build_object(
    ...
    'active_recovery_key_id', active_recovery_key_id
  );
end;
$complete$;
