...
5. The PWA caches the app shell. Public key-derivation metadata, an encrypted verifier, encrypted quotes, and author names are cached for offline use. First use requires an online sign-in and preparation of the local vault. Afterward, a saved account and encrypted verifier allow offline unlock even when the access token has expired or session renewal is unreachable. Pending changes synchronize while the app is open after its session reconnects.

A reload always requires the shared vault passphrase again: neither the passphrase nor the derived encryption key is persisted. Local access does not authenticate server requests. Sign-out and observed session/membership denials lock the local vault and invalidate offline preparation. A disconnected device cannot learn of remote revocation or a vault reset until it reconnects.
