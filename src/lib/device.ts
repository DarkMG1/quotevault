...
const base64urlBytes = (value: unknown, expected: number): boolean => {
    if (typeof value !== 'string' || !BASE64URL.test(value) || value.length % 4 === 1) return false;
    try { const standard = `${value.replace(/-/g, '+').replace(/_/g, '/')}${'==='.slice((value.length + 3) % 4)}`; const bytes = new Uint8Array(base64ToArrayBuffer(standard)); return bytes.byteLength === expected && arrayBufferToBase64(bytes).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '') === value; } catch { return false; }
};
...
const validProtection = (value: unknown): Record<string, unknown> => { const protection = objectValue(value, 'protection'); if (new TextEncoder().encode(canonicalJson(protection)).byteLength > 32768) invalid('Invalid device protection.'); return protection; };
...
export async function enrollmentFingerprint(input: { accountId: string; publicKeyFingerprint: string; tokenDigest: string; protectionMode: 'passkey-prf' | 'remembered'; protection: Record<string, unknown> }): Promise<string> {
    ...
    const payload = `[1,${JSON.stringify(input.accountId)},${JSON.stringify(input.publicKeyFingerprint)},${JSON.stringify(input.tokenDigest)},${JSON.stringify(input.protectionMode)},${canonicalJson(validProtection(input.protection))}]`;
...
export async function requestDevice(input: RequestDeviceInput): Promise<{ requestId: string; deviceId: string; enrollmentFingerprint: string; expiresAt: string }> {
    ...
    const { jwk, fingerprint } = await publicKey(input.publicJwk, input.publicKeyFingerprint); if (!base64urlBytes(input.tokenDigest, 32)) invalid('Invalid device token digest.'); const protection = validProtection(input.protection); const bundle = envelope(input.encryptedPrivateBundle); const enrollment = await enrollmentFingerprint({ accountId: ownerId, publicKeyFingerprint: fingerprint, tokenDigest: input.tokenDigest, protectionMode: mode(input.protectionMode), protection });
...
export async function getDeviceRequest(requestId: string): Promise<DeviceRequest> {
    ...
    const ownerId = uuidValue(data.owner_id, 'owner ID'); const { jwk, fingerprint } = await publicKey(data.public_jwk, stringValue(data.public_key_fingerprint, 'public-key fingerprint')); const tokenDigest = stringValue(data.authorization_token_digest, 'token digest'); if (!base64urlBytes(tokenDigest, 32)) invalid('Invalid device token digest.'); const protectionMode = mode(data.protection_mode); const protection = validProtection(data.protection); const enrollment = await enrollmentFingerprint({ accountId: ownerId, publicKeyFingerprint: fingerprint, tokenDigest, protectionMode, protection });
...
export async function completeDevice(accountId: string, deviceId: string, token: string, rememberedKey?: CryptoKey, generation?: string): Promise<DeviceLocalState> {
    const account = uuidValue(accountId, 'account ID'); const device = uuidValue(deviceId, 'device ID'); const loaded = await loadDeviceState(account); if (!loaded || loaded.deviceId !== device) invalid('Device enrollment state is missing.'); const current = loaded as DeviceLocalState; if (current.protectionMode === 'remembered' && (!rememberedKey || rememberedKey.extractable !== false)) invalid('Remembered device keys must be non-extractable.'); if (current.protectionMode === 'passkey-prf' && rememberedKey) invalid('Passkey device cannot persist a transient key.'); const targetGeneration = generation ?? current.wrapper?.generation; if (!targetGeneration) invalid('Device generation is missing.');
    const data = objectValue(await rpc('complete_device', { p_device_id: device, p_token: token, p_generation: uuidValue(targetGeneration, 'generation') }), 'completion response'); if ('lease' in data) invalid('Unsigned device lease rejected.'); const responseDevice = uuidValue(data.device_id, 'device ID'); const responseGeneration = uuidValue(data.generation, 'generation'); if (responseDevice !== device || responseGeneration !== targetGeneration) invalid('Invalid device completion response.'); const next: DeviceLocalState = { accountId: account, deviceId: device, protectionMode: current.protectionMode, protection: current.protection, encryptedPrivateBundle: envelope(current.encryptedPrivateBundle), wrapper: { generation: responseGeneration, wrappedKey: strictWrapper(data.wrapped_key) } }; if (current.protectionMode === 'remembered') next.rememberedKey = rememberedKey; expiry(data.lease_expires_at); await saveDeviceState(next); return next;
}
...
const sanitize = (value: unknown, accountId: string): DeviceLocalState => { const data = objectValue(value, 'local state'); if (uuidValue(data.accountId, 'account ID') !== accountId) invalid('Device state account mismatch.'); const protectionMode = mode(data.protectionMode); const result: DeviceLocalState = { accountId, deviceId: uuidValue(data.deviceId, 'device ID'), protectionMode, protection: validProtection(data.protection), encryptedPrivateBundle: envelope(data.encryptedPrivateBundle) }; if (data.rememberedKey !== undefined) { if (protectionMode === 'passkey-prf' || !data.rememberedKey || typeof data.rememberedKey !== 'object' || (data.rememberedKey as CryptoKey).extractable !== false) invalid('Invalid remembered device key.'); result.rememberedKey = data.rememberedKey as CryptoKey; } if (data.wrapper !== undefined) { const wrapper = objectValue(data.wrapper, 'wrapper'); result.wrapper = { generation: uuidValue(wrapper.generation, 'generation'), wrappedKey: strictWrapper(wrapper.wrappedKey) }; } if (data.lease !== undefined) result.lease = structuralLease(data.lease); return result; };
export async function saveDeviceState(state: DeviceLocalState): Promise<void> { await db.deviceState.put(sanitize(state, state.accountId)); }
export async function loadDeviceState(accountId: string): Promise<DeviceLocalState | null> { uuidValue(accountId, 'account ID'); const value = await db.deviceState.get(accountId); return value == null ? null : sanitize(value, accountId); }
