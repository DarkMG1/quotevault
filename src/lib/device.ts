...
export const recoverySetupConflictMessage = (replace: boolean, state: Pick<DeviceLocalState, 'recoverySetupRequired'>): string | null => !replace && !needsRecoverySetup(state) ? 'Recovery was configured on another device. This displayed phrase was not saved; reload to continue.' : null;
