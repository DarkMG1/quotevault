...
export async function revokeOwnDevice(deviceId: string, token: string, targetDeviceId = deviceId): Promise<{ deviceId: string; status: 'revoked' }> { const actor = uuidValue(deviceId, 'device ID'); const id = uuidValue(targetDeviceId, 'device ID'); if (!base64urlBytes(token, 32)) invalid('Invalid device token.'); const data = objectValue(await rpc('revoke_own_device', { p_device_id: actor, p_token: token, p_target_device_id: id }), 'revocation response'); if (data.status !== 'revoked' || uuidValue(data.device_id, 'device ID') !== id) invalid('Invalid device revocation response.'); return { deviceId: id, status: 'revoked' }; }
