    ...
    return <main className="min-h-[100dvh] flex items-center justify-center bg-background p-4">
        <section className="w-full max-w-sm bg-surface p-8 rounded-3xl border border-slate-700 text-center">
            ...
            {legacy ? <form onSubmit={onLegacyUnlock} className="space-y-4">
                ...
                {(pending || preparingPending) && approvalUrl && <div className="space-y-2"><p className="font-mono text-xl tracking-wider text-white">{approvalCode}</p>{qr && <img src={qr} alt="Device approval QR code" className="mx-auto bg-white p-2 rounded-xl" />}<a href={approvalUrl} className="block text-xs text-primary-400 break-all">Open approval request</a></div>}
