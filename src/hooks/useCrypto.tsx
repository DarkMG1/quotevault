...
export const CryptoProvider = ({ children }: { children: ReactNode }) => {
    ...
    const enroll = useCallback(async (mode: 'remembered' | 'passkey-prf') => {
        if (!user || !stateRef.current || stateRef.current.envelope_status === 'legacy' || !canSync) { const message = stateRef.current?.envelope_status === 'legacy' ? 'Prepare the vault migration before requesting a device.' : 'Connect to request a device.'; setError(message); throw new Error(message); } setBusy(true); setError('');
        ...
        } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not request this device.'); throw cause; } finally { setBusy(false); }
