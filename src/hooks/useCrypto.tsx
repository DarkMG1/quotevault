...
import { completeDevice, deleteDeviceState, formatEnrollmentCode, getDeviceRequest, loadDeviceState, needsRecoverySetup, recoverySetupConflictMessage, revokeOwnDevice, saveDeviceState } from '../lib/device';
...
export const CryptoProvider = ({ children }: { children: ReactNode }) => {
    ...
    const setupRecovery = useCallback(async (phrase: string, replace = false) => {
        ...
        try { const encryptedPrivateKey = await encryptPrivateBundle({ version: 1, privateJwk, authorizationToken: token }, encryptionKey, { accountId: user.id, recordId: recoveryKeyId, publicKeyFingerprint, protectionMode: 'recovery', version: 1, recoveryKdf: kdf }); const wrappedKey = await wrapVaultKey({ version: 1, vaultId: 'quotevault', generation: vault.generation, targetFingerprint: publicKeyFingerprint, masterKey: master }, pair.publicKey); const auth = await getDeviceAuthorization(); try { await createRecoveryKey({ recoveryKeyId, publicJwk, publicKeyFingerprint, encryptedPrivateKey, kdf, generation: vault.generation, wrappedKey, deviceId: auth.deviceId, token: auth.token }, replace); } catch (cause) { const local = await loadDeviceState(user.id); let completed: DeviceLocalState | null = null; try { if (local) completed = await completeDevice(user.id, local.deviceId, auth.token, local.rememberedKey, vault.generation); } catch { /* The original creation failure is more useful. */ } const message = completed ? recoverySetupConflictMessage(replace, completed) : null; if (message) { setDeviceState(completed); throw new Error(message); } throw cause; } const local = await loadDeviceState(user.id); if (local?.recoverySetupRequired) { await saveDeviceState({ ...local, recoverySetupRequired: false }); setDeviceState({ ...local, recoverySetupRequired: false }); } }
