...
export const CryptoProvider = ({ children }: { children: ReactNode }) => {
    ...
    const unlockDevice = useCallback(async (mode: 'remembered' | 'passkey-prf') => {
        ...
        try {
            ...
            if (unlockRequest.current !== version) { opened.fill(0); return; } clearKeys(); masterKey.current = opened; keyGeneration.current = vault.generation; setDeviceState(local); setLeaseExpired(false); setLeaseExpiresAt(local.lease?.claims[5] ?? null); setRecoveryRequired(!!local.recoverySetupRequired); setEncryptionKey(quoteKey);
