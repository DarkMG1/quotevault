...
psql -X -v ON_ERROR_STOP=1 -d "$test_db" -c "do \$\$ begin if has_function_privilege('anon',to_regprocedure('public.begin_legacy_reversion(uuid,bigint,uuid,text)'),'execute') or has_function_privilege('anon',to_regprocedure('public.stage_legacy_reversion(uuid,jsonb,uuid,text)'),'execute') or has_function_privilege('anon',to_regprocedure('public.commit_legacy_reversion(uuid,jsonb,jsonb,uuid,text)'),'execute') or has_function_privilege('authenticated',to_regprocedure('public.qv_reversion_authorized(public.vault_state,uuid,text)'),'execute') or has_function_privilege('authenticated',to_regprocedure('public.qv_legacy_v1_text(text)'),'execute') or has_table_privilege('authenticated','public.vault_legacy_reversions','select,insert,update,delete') or has_table_privilege('anon','public.vault_legacy_reversions','select,insert,update,delete') or has_table_privilege('authenticated','public.vault_legacy_reversion_rows','select,insert,update,delete') or has_table_privilege('anon','public.vault_legacy_reversion_rows','select,insert,update,delete') then raise exception 'legacy reversion privilege is too broad'; end if; end \$\$;"
